...
extern char Light_state;	//灯光运行状态

void WS_Load(void) { HAL_TIM_PWM_Start_DMA(&htim4, TIM_CHANNEL_1, (uint32_t*)send_Buf, NUM); }
...
void LED_RUN(void)
{
	if(Light_state == 1)
	{
		WS_WriteAll_RGB(c_red*8,c_green*4,c_blue*8);
	}
	else
	{
		WS_CloseAll();
